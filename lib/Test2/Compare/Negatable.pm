...
sub toggle_negate {
    my $self = shift;
    $self->set_negate(!$self->negate ? 1 : 0);
}
...
Test2::Compare::Negatable - Poor mans 'role' for negatable compare objects.
...
so we cannot use inheritence for the behavior we actually want. This module
...
Toggle the negate atttribute. If the attribute was on it will now be off, if it
